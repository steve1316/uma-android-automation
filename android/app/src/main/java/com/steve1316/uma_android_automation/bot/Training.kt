...
open class Training(protected val game: Game, protected val campaign: Campaign) {
    ...
    private val unityCupExtremeBurstMinStatGain: Int = SettingsHelper.getIntSetting("scenarioOverrides", "unityCupExtremeBurstMinStatGain", 0)

    /** Unity Cup only: after Junior Year, only prioritize Spirit Explosion bursts (normal and extreme) on facilities whose stat is in the top 3 prioritized stats. false (default) is unrestricted. */
    private val unityCupBurstTopStatsOnlyAfterJunior: Boolean = SettingsHelper.getBooleanSetting("scenarioOverrides", "unityCupBurstTopStatsOnlyAfterJunior", false)
    ...
    companion object {
        ...
        /**
         * Whether a Spirit Explosion burst (normal or extreme) should be prioritized for `stat`. When the "top stats only after Junior" override is on, a burst is only allowed after
         * Junior Year on facilities whose stat is in the top 3 prioritized stats. Junior Year and the disabled default are always allowed. Pure and unit-testable.
         *
         * @param enabled Whether the "burst only top 3 stats after Junior" override is on.
         * @param year The current career year.
         * @param prioritization The ordered stat prioritization list (already empty-normalized to all stats).
    ...
    fun analyzeTrainings(args: Map<String, Any?> = emptyMap()) {
        ...
        // The whole-turn train/rest decision is made from the full analysis below, not from this single Speed probe. Speed is the
        // first facility shown, but the code models Wit as the safest (normalizeFailureChances excludes it), so a high Speed reading
        // does not mean every facility is too risky. Analyze all five and let handleTraining rest only when trainingMap ends up empty
        // (every facility filtered out). The Speed reading above still seeds the per-facility OCR-misread fallback.
        if (!test) {
            MessageLog.i(TAG, "[TRAINING] Initial Speed failure chance $failureChance%. Analyzing all 5 trainings...")
        }
        ...
        if (!singleTraining && analysisResults.isNotEmpty()) {
            ...
            // Every facility was rejected by the per-training filters. Mirror handleTraining's own reasoning: treat it as an
            // energy problem (recover energy) unless the emptiness is purely restriction/blacklist, which resting cannot fix.
            if (!test && !singleTraining && trainingMap.isEmpty()) {
                needsEnergyRecovery = !allTrainingsRestrictedOrBlacklisted
            }
        ...
        if (ButtonTraining.click(game.imageUtils)) {
            ...
            if (trainingMap.isEmpty()) {
                ...
                if ((trainWitDuringFinale && campaign.date.day > 72) || campaign.date.day == 75) {
                    ...
                    if (campaign.checkMainScreen()) {
                        val recoverReason =
                            if (allTrainingsRestrictedOrBlacklisted) {
